    ...
    class SplitLargeFiles extends Plugin {
      onStart() {
        ...
        this.messageContextMenuUnpatch = ContextMenu.patch("message", (tree, props) => {
          ...
          tree.props.children[2].props.children.push(ContextMenu.buildItem({ type: "separator" }), ContextMenu.buildItem({
            label: "Refresh Downloadables", action: () => {
              this.findAvailableDownloads();
              BdApi.showToast("Downloadables refreshed", { type: "success" });
            }
          }), ContextMenu.buildItem({
            label: "Copy Download Links", action: () => {
          ...
          if (incomplete && this.canDeleteDownload(incomplete)) {
            tree.props.children[2].props.children.push(ContextMenu.buildItem({
              label: "Delete Download Fragments", danger: true, action: () => {
          ...
          if (!incomplete) {
            tree.props.children[2].props.children.push(ContextMenu.buildItem({
              label: "Download Large File", action: () => {
